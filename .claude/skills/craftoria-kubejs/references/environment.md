...
at a time, document why in the change summary, run `sync --dry-run`, rebuild the ZIP, and run the
installer with `--dry-run` before real application. Existing KubeJS replacement hashes belong in
`payload-policy.json`.
