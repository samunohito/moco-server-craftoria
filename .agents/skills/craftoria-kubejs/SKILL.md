...
8. Use `tools/craftoria-overlay` for tracked payload changes; update only explicit manifest-owned files and retain hash verification/backups.
9. Build distribution ZIPs, bump the add-on version, and run packaged-installer checks only when the user explicitly requests a release or distribution artifact. Ordinary payload development requires focused validation and `sync --dry-run`, not a release build.
