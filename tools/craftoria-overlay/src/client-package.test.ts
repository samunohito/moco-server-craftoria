...
import { lolipopOutputPath, writeLolipopArchive } from './export-lolipop.js';
...
test('Lolipop export patches the WebGUI-managed NeoForge directory without replacing its launcher', async () => {
  ...
  try {
    ...
    const runtimeConfigContent = '# regenerated by the mod\nconfig value\n';
    ...
    const configSource = path.join(scratch, 'overlay-source.toml');
    await writeFile(configSource, configContent, 'utf8');
    await writeFile(path.join(server, 'config/test-common.toml'), runtimeConfigContent, 'utf8');
    ...
    await writeFile(path.join(server, 'mods/old.jar'), 'old mod', 'utf8');
    await writeFile(
    ...
    await writeLolipopArchive({
      sourceServer: server,
      outputPath: output,
      manifest,
      overlaySources: new Map([['config/test-common.toml', configSource]]),
    });
    const entries = await zipEntries(output);
    const names = entries.map(({ name }) => name);
    const prefix = 'Craftoria-Lolipop-NeoForge-Patch-9.8.7-test/';
    assert.ok(names.length > 0);
    assert.ok(names.every((name) => name.startsWith(prefix)));
    ...
    assert.ok(!names.some((name) => name.startsWith(`${prefix}libraries/`) || name.startsWith(`${prefix}versions/`)));
...
test('Lolipop export defaults to a provider-specific artifact name', () => {
  ...
  assert.equal(
    ...
    path.join(resourceRoot, 'dist', 'Craftoria-Lolipop-NeoForge-Patch-9.8.7-test.zip'),
  );
});
